...
def block_merge_determiner(previous_block, current_block):
    ...
    if (vertical_difference <= 2.5) and (horizontal_difference <= 2.5) :
        return True
    else:
        return False
    ...
    for index,row in block.iterrows():
        if index==0:
            ...
            if block_merge_determiner(prev_block,curr_block):
                ...
                prev_index = curr_index
                prev_block = curr_block
    # block.to_csv('block_aegon.csv',index=False)
    # line.to_csv('line_aegon.csv',index=False)
    # word.to_csv('word_aegon.csv',index=False)
    return block, line, word
"""